...
def read_count_data(input_file, inv_interval, stat, total_length):
    ...
    统计 (g2, g1) 对的数量。并以该块中点（或第一 g1 的中点）归一化作为输入 x。
    返回列表 [(x, count), ...]
    """
    blocks = []
    with open(input_file) as f:
    ...
    for i in range(len(indices)-1):
        ...
        g1_seg = gene_block[0]
        x = ((g1_seg[1] + g1_seg[2])//2 - stat) / total_length
...
def deep_simulate_regions(stat, total_length, length_models, count_model, g3min, g3max):
    """
    深度学习模式下模拟生成 slim 调用：
      - g3（非编码）：采用 predict_length，fallback为均匀采样 [g3min, g3max]
      - g1（外显子）：采用 predict_length，fallback为对数正态（mean=50, sigma=log(2)）
      - g2（内含子）：采用 predict_length，fallback为对数正态（mean=100, sigma=log(1.5)），最低10
      - gene块中数量由 predict_count (fallback使用 fallback_count) 预测
    模拟流程：
      while(base < total_length):
          生成一段 g3
          如果 base < total_length:
              生成第一段 g1
              预测count = 模型预测（或 fallback）
    ...
    fb_g1 = lambda: int(np.random.lognormal(np.log(50), np.log(2))) + 1
    fb_g2 = lambda: int(np.random.lognormal(np.log(100), np.log(1.5))) + 10
...
def main():
    parser = argparse.ArgumentParser(
        description="学习模式脚本（-learn）：通过输入已有 slim 文件学习各区段长度和 g1/g2 数量分布，"
                    ...
                    " -g3min 与 -g3max 为 fallback g3 参数（可选，不输入则使用默认 100 和 5000）。"
    )
    ...
    parser.add_argument("-ep", type=int, default=20, help="训练迭代次数，默认20")
    parser.add_argument("-g3min", type=int, default=100, help="fallback 非编码区最小长度，默认100")
    parser.add_argument("-g3max", type=int, default=5000, help="fallback 非编码区最大长度，默认5000")
    ...
    sim_segments = deep_simulate_regions(stat, total_length, length_models, count_model, args.g3min, args.g3max)
